...
def read_from_academics_db(academics_q1, academics_q2, academics_q3):
   ...
   if (academics_q2 != 'Yes'and academics_q3 != 'Yes'):
      ...
      for i in [1,2,3]:
         ...
         if response['Items'][0].has_key('link'):
            responses.append ([response['Items'][0]['content'],response['Items'][0]['link']])
         else: 
            responses.append ([response['Items'][0]['content'],"null"])
      return responses
   else:
      responses = []
      responses.append (['Keep up the great work!',"null"])
      return responses
...
def read_from_physical_db(physical_q1, physical_q2, physical_q3):
   ...
   if physical_q1 == 'ThirtyToSixty' or physical_q1 == 'PlusSixty':
      responses.append(['You need more exercise!',"kpe.utoronto.ca"])
   return responses
